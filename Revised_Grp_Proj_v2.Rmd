...
```{r, include=FALSE}
library(mlbench)
Credit <- read.csv("https://raw.githubusercontent.com/JZhong01/STA490/main/Week5/crx.data", header = TRUE)
...
Credit = Credit %>% rename(Gender = b, Age = X30.83, Debt = X0, Married = u, BankCustomer = g, Industry = w, Ethnicity = v, YearsEmployed = X1.25, PriorDefault = t, Employed = t.1, CreditScore = X01, DriversLicense = f, Citizen = g.1, ZipCode = X00202, Income = X0.1, Approved = X.)

Credit = Credit %>% filter(!apply(Credit == "?", 1, any))


Credit = Credit %>% mutate(Gender = ifelse(Gender == "a", 1, 0))
Credit = Credit %>% mutate(Age = as.numeric(Age))
Credit = Credit %>% mutate(Married = ifelse(Married == "u", 1, 0))
Credit = Credit %>% mutate(BankCustomer = ifelse(BankCustomer == "g", 1, 0))

Credit = Credit %>% mutate(Ethnicity = case_when(
    Ethnicity == "h" ~ "Black",
    Ethnicity == "v" ~ "White",
    Ethnicity == "bb" ~ "Asian",
    Ethnicity == "ff" ~ "Latino",
...
Credit = Credit %>% mutate(PriorDefault = ifelse(PriorDefault == "t", 1, 0))
Credit = Credit %>% mutate(Employed = ifelse(Employed == "t", 1, 0))
Credit = Credit %>% mutate(DriversLicense = ifelse(DriversLicense == "t", 1, 0))
Credit = Credit %>% mutate(Citizen = case_when(
    Citizen == "g" ~ "ByBirth",
    Citizen == "s" ~ "ByOtherMeans",
    Citizen == "p" ~ "Temporary"
))
Credit = Credit %>% mutate(ZipCode = as.numeric(ZipCode))
Credit = Credit %>% mutate(ZipCode = case_when(
   ZipCode > 0 & ZipCode <= 199 ~ "North",
   ZipCode >= 200 & ZipCode <= 399 ~ "South",
   ZipCode >= 700 & ZipCode <= 799 ~ "South",
   ZipCode >= 400 & ZipCode <= 599 ~ "Midwest",
   ZipCode >= 800 & ZipCode <= 999 ~ "West",
   TRUE ~ "Unassigned"
))

Credit = Credit %>% mutate(Approved = ifelse(Approved == "+", 1, 0))

Credit = Credit %>% mutate(CreditScore = as.numeric(CreditScore))
...
full.model <- glm(Approved ~ grp.Debt + grp.Age + grp.Income + Gender + BankCustomer + Ethnicity + grp.YearsEmployed + PriorDefault + Employed + DriversLicense + Citizen + ZipCode + CreditScore,
...
# Fit the logistic regression model
reduced.model <- glm(Approved ~ grp.Income + grp.Debt + Employed + BankCustomer + PriorDefault + CreditScore, 
...
## automatic variable selection
library(MASS)
final.model.both <- stepAIC(reduced.model, 
direction = "both",   trace = 0   
)

# Create a summary table
summary_table_final <- as.data.frame(summary(final.model.both)$coef)
...
goodness <- rbind(full.model = global.measure(full.model),
                  ...
                  final.model = global.measure(final.model.both))
...
model.coef.stats <- summary(final.model.both)$coef
odds.ratio <- exp(coef(final.model.both))
